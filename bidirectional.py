...
from keras.layers import Input, LSTM, Dense, Dropout, Lambda, GRU, Bidirectional, BatchNormalization
...
with open("fasttext_bootstrapping_800000.txt", "r", encoding="utf-8") as file:
    for line in file:
        words = line.split()
...
def convert_sentence_to_list_of_vectors(sentence: Union[str, float]):
    if type(sentence) == str:
        return [from_word_to_vector[word] for word in sentence.split() if word in from_word_to_vector]
    else:
        return []
...
dataset = pd.read_csv("data/bootstrapping_800000.tsv", names=["context_id", "context_2", "context_1", "context_0",
...
dataset.confidence = dataset.confidence / 0.5 * 0.8591

del from_word_to_vector
gc.collect()
...
model.load_weights("weights_bootstrapping_800000_1_epoch.hdf5")
...
model.load_weights("weights_bootstrapping_800000_2_epochs.hdf5")
