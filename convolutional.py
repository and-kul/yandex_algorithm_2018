...
from keras.layers import Input, LSTM, Dense, Dropout, Lambda, GRU, Conv1D, MaxPooling1D
...
shared_conv1D = Conv1D(filters=300, kernel_size=3, padding="same", activation="relu")
shared_pooling = MaxPooling1D(pool_size=2)

after_conv_and_pooling_context_2 = shared_pooling(shared_conv1D(get_context_2))
after_conv_and_pooling_context_1 = shared_pooling(shared_conv1D(get_context_1))
after_conv_and_pooling_context_0 = shared_pooling(shared_conv1D(get_context_0))
after_conv_and_pooling_reply = shared_pooling(shared_conv1D(get_reply))

shared_gru = GRU(200)

encoded_context_2 = shared_gru(after_conv_and_pooling_context_2)
encoded_context_1 = shared_gru(after_conv_and_pooling_context_1)
encoded_context_0 = shared_gru(after_conv_and_pooling_context_0)
encoded_reply = shared_gru(after_conv_and_pooling_reply)
